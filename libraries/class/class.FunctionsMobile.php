...
class FunctionsMobile
{
	...
	public function price($price,$unit=null){
		return empty($this->format_money($price,$unit))?'Liên Hệ':$this->format_money($price,$unit);
	}
	/* Check recaptcha */
	...
	public function checkMemberLogin(){
		global $login_member;
		return isset($_SESSION[$login_member]['active']) && $_SESSION[$login_member]['active'];
	}
	/* Lấy youtube */
	...
	{
		$x = (file_exists($patch) && !is_dir($patch)) ? getimagesize($patch) : null;
	...
	// public function get_product_color($id_product,$id_mau){
	// 	global $colors;
	// 	$color = Product::get_mau($id_mau);
	// 	$color_image = Product::get_product_mau_images($id_product,$id_mau);
	// 	$data_photo2 = '';
	// 	if(!empty($color_image['photo'])){ 
	// 		$data_photo2 = THUMBS.'/280x260x2/'.UPLOAD_PRODUCT_L.$color_image['photo'];
	// 	} 
	// 	return '<div class="color_item" data-src="'.$data_photo2.'"  style="background-color:#'.$colors[$id_mau]["mau"].';"></div>';
	// }
	public function get_size_slide_product($info){
		global $sluglang,$lang,$sizes,$d,$popup_size;
		if(empty($info)) return false;
		$id_product = $info['id_product'];
		$size_str = $info['id_size'];
		?>
		<?php if(!empty($size_str)){
			$id_sizes = explode(",", $size_str);
			?>
			<div class="product-item__size-slide">
				<?php foreach($id_sizes as $mid){ ?>
					<div class="size_item size_<?=$mid?>" data-idproduct="#product-item-<?=$id_product?>" data-idsize="<?=$mid?>"><?=$sizes[$mid]["ten$lang"]?></div>
				<?php } ?>
				<div class="d-inline-block size-guild" style="top: 11px;
				position: relative;
				text-decoration: underline;">
				<a href="<?=UPLOAD_PHOTO_L.$popup_size["photo"]?>" data-fancybox="images" class="text-secondary">
					<div>
						<span class="d-block">Size</span><span class="d-block">Guild</span> </a></div></div>
					</div>
				<?php } ?>
			<?php }

			public function get_size_slide($info){
				global $sluglang,$lang,$sizes,$d;
				if(empty($info)) return false;
				$id_product = $info['id_product'];
				$size_str = $info['id_size'];
				?>
			...
			public function get_colors_slide($info){
				global $sluglang,$lang,$colors,$d;
				if(empty($info)) return false;
				$color_str = $info['id_color'];
				$id_product = $info['id_product'];

				$color_images= $d->rawQuery("select id_mau,photo,id_photo as id_product from #_gallery where id_mau > 0 and id_photo=? and type=? and kind='man' and val='san-pham' and com='product' ",array($id_product,'san-pham'));
				?>
				<?php if(!empty($color_str)){
					$id_mau = explode(",", $color_str);
					?>
					<div class="product-item__color-slide owl-carousel">
						<?php foreach($color_images as $mid){
							if(!empty($mid['photo'])){ 
								$data_photo2 = THUMBS.'/280x260x1/'.UPLOAD_PRODUCT_L.$mid['photo'];
							} 
							?>
							<span class="color_item_<?=$mid['id_mau']?>">
								<div class="color_item" data-idmau="<?=$mid['id_mau']?>" data-idproduct="#product-item-<?=$id_product?>" data-src="<?=$data_photo2?>"  style="background-color:#<?=$colors[$mid['id_mau']]["mau"]?>;"></div>
							</span>
						<?php } ?>
					</div>
				<?php } ?>
			<?php }
			public function get_product_item_batdongsan($item){
				global $sluglang,$lang,$all_colors;
				$data_photo2 = '';
				if(!empty($item['photo2'])){ 
					$data_photo2 = THUMBS.'/280x230x1/'.UPLOAD_PRODUCT_L.$item['photo2'];
				}
				?>
				<div class="product-bds-item default-hover overlay-wrapper mb-4" id="product-item-<?=$item["id"]?>">
					<div class="product-item__image image" data-src='<?=$data_photo2?>'>
						<a class="text-decoration-none" href="<?=$item[$sluglang]?>" title="<?=$item['ten'.$lang]?>">
							<img onerror="this.src='<?=THUMBS?>/280x230x1/assets/images/noimage.png';" src="<?=THUMBS?>/280x230x1/<?=UPLOAD_PRODUCT_L.$item['photo']?>" alt="<?=$item['ten'.$lang]?>" class="img-fluid w-100"/>
						</a>
					</div>
					<div class="product-item__detail detail overlay-item">
						<div class="overlay-item-wrap">	
							<div class="my-auto">
								...
								<div class="mota line-limit to-4">
									<?=$item["mota$lang"]?>
								</div>
							</div>
						</div>
			...
			public function get_product_item($item){
				...
				?>
				<div class="product-item default-hover  mb-4" id="product-item-<?=$item["id"]?>">
					<div class="product-item__image image" data-src='<?=$data_photo2?>'>
						<a class="text-decoration-none" href="<?=$item[$sluglang]?>" title="<?=$item['ten'.$lang]?>">
							<img onerror="this.src='<?=THUMBS?>/280x230x1/assets/images/noimage.png';" src="<?=THUMBS?>/280x230x1/<?=UPLOAD_PRODUCT_L.$item['photo']?>" alt="<?=$item['ten'.$lang]?>" class="img-fluid w-100"/>
						</a>
					...
					<div class="product-item__detail detail">
						...
						<div class="price">
							Giá: <span class="gia"><?=$this->price($item['gia'])?></span>
						</div>
					</div>
				</div>
			<?php }
			...
			public function get_mxh($social,$w=40,$h=40){
				...
				{
					$socialString .= '<a href="'.$value['link'].'" target="_blank"><img onerror="this.src=\'thumbs/'.$w.'x'.$h.'x1/assets/images/default/noimage.png\';" src="'.$config_base.UPLOAD_PHOTO_L.@$value['photo'].'" style="max-height:'.$h.'px;" /></a>';
				}
				return $socialString;
			}
			public function get_product_item_by_type($item,$type){
				switch ($type) {
			...
			public function get_product_tpl_mobile($product){
				...
				<div class="row">
					<?php foreach($product as $item2){ ?>
						<div class="col-md-3 col-sm-4 col-6">
							<?=$this->get_product_item($item2);?>
						</div>
					<?php } ?>
				</div>
			<?php }

			public function get_news_item($item,$type='dichvu'){
				global $sluglang,$lang;
				?>
				<div class="index-news-item <?=$type?>-item default-hover">
					<div class="image">
						<a class="text-decoration-none" href="<?=$item[$sluglang]?>" title="<?=$item['ten'.$lang]?>">
					...
					<div class="detail">
						...
						<div class="time-news"><?=ngaydang?>: <?=date("d/m/Y h:i A",$item['ngaytao'])?></div>
						<div class="desc-news line-limit to-3"><?=$item['mota'.$lang]?></div>
						<a href="lien-he" class="btn-lienhe">Liên Hệ</a>
					</div>
				</div>
			<?php }
		}
