<?php
	...
	if(isset($_POST['thanhtoan']))
	{
		...
		$tamtinh = $cart->get_order_total_tamtinh();
		$ship = $cart->get_order_total_ship();
	    ...
	    for($i=0;$i<$max;$i++)
	    {
			...
			$unit_price = $cart->get_product_gia($pid,$quanprice);
			$textsm='';
	    	...
	    	$chitietdonhang.='</td>';
	    	$chitietdonhang.='<td align="left" style="padding:3px 9px" valign="top"><span style="color:#ff0000;">'.$func->format_money($unit_price).'</span></td>';
	    	$chitietdonhang.='<td align="center" style="padding:3px 9px" valign="top">'.$q.'</td>';
	    	
	    	$chitietdonhang.='<td align="right" style="padding:3px 9px" valign="top"><span style="color:#ff0000;">'.$func->format_money($unit_price*$q).'</span></td>';
		...
		if($id_insert)
		{
			for($i=0;$i<$max;$i++)
			{
				...
				$quanprice = $_SESSION['cart'][$i]['quanprice'];
				$proinfo = $cart->get_product_info($pid);
				$gia = $cart->get_product_gia($pid,$quanprice);
				$giacu = $proinfo['giacu'];
				...
				$data_donhangchitiet['id_mau'] = $_SESSION['cart'][$i]['mau'];
				$data_donhangchitiet['id_size'] = $_SESSION['cart'][$i]['size'];
				$data_donhangchitiet['id_quanprice'] = $_SESSION['cart'][$i]['quanprice'];
