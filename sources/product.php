<?php  
	...
	if($id!='')
	{
		...
		$row_quanprice = $d->rawQuery("select * from #_product_quanprice where id_product= ?",array($row_detail['id']));
	...
	else
	{
		...
		$params = array($type);
		if($com=='bo-suu-tap'){
			$where .= " and find_in_set(?,id_brand) ";
			$params = array($type);
		}

		if($com=='khuyen-mai'){
			$where .= " and gia > 0 and giacu > 0 ";
			$params = array($type);
		...
		$startpoint = ($curPage * $per_page) - $per_page;
		$limit = " limit ".$startpoint.",".$per_page;
		$sql = "select *  from #_product where $where order by stt asc ,id desc $limit";
		$product = $d->rawQuery($sql,$params);
