		...
		<div class="card card-primary card-outline text-sm">
			...
			<div class="card-body">
				<div class="row">
					...
					<?php 
					$all_group = $d->rawQuery("select id,title from #_member_group where hienthi=1 ");
					?>			
					<div class="form-group col-md-4">
						<label for="member_group">Nhóm Thành Viên:</label>
						<select class="form-control" name="data[id_member_group]" id="id_member_group">
							<option value="0">Chọn nhóm</option>
							<?php foreach($all_group as $group){ ?>
								<option <?=(@$item['id_member_group']==$group['id'])?"selected":""?> value="<?=$group['id']?>"><?=$group["title"]?></option>
							<?php } ?>
