<?php
$count_other_photo = 0;
function get_bosuutap($id=0)
	{
		global $d, $type;

		if($id)
		{
			$temps = $d->rawQueryOne("select id_brand from #_product where id = ? and type = ? limit 0,1",array($id,$type));
			$arr_mau = explode(',', $temps['id_brand']);
			
			for($i=0;$i<count($arr_mau);$i++) $temp[$i]=$arr_mau[$i];
		}

		$row_mau = $d->rawQuery("select tenvi, id from #_product_brand where type = ? order by stt,id desc",array($type));

		$str = '<select id="brand_group" name="brand_group[]" class="select multiselect" multiple="multiple" >';
    ...
    if(
    	...
    	(isset($config['product'][$type]['photo2']) && $config['product'][$type]['photo2'] == true)|| 
    	(isset($config['product'][$type]['photo3']) && $config['product'][$type]['photo3'] == true)|| 
    	(isset($config['product'][$type]['photo4']) && $config['product'][$type]['photo4'] == true))
    {
    	$colLeft = "col-xl-8";
    	$colRight = "col-xl-4";
    }
...
<section class="content">
    <form class="validation-form" novalidate method="post" action="<?=$linkSave?>" enctype="multipart/form-data">
        ...
        <div class="row">
        	...
        	<div class="<?=$colRight?>">
			            ...
			            <div class="card-body">
		            		<div class="form-group-category row">
					            ...
					            <?php if(isset($config['product'][$type]['brand']) && $config['product'][$type]['brand'] == true) { ?>
							    	<div class="form-group col-xl-6 col-sm-4">
					                    <label class="d-block" for="id_brand">Danh mục <?=$config['product'][$type]['title_main_brand']?>:</label>
					                    <?php //$func->get_ajax_category('product', 'brand', $type, 'Chọn Collection')?>
					                    <?=get_bosuutap(@$item['id'])?>
					                </div>
							    <?php } ?>
				...
				<?php if(isset($config['product'][$type]['images']) && $config['product'][$type]['images'] == true) { ?>
			        <div class="card card-primary card-outline text-sm">
			            <div class="card-header">
			                <h3 class="card-title">Hình ảnh <?=isset($config['product'][$type]['title_photo']) ?$config['product'][$type]['title_photo']: ++$count_other_photo?></h3>
			                <div class="card-tools">
			                	<button type="button" class="btn btn-tool" data-card-widget="collapse"><i class="fas fa-minus"></i></button>
			                </div>
			            </div>
		        ...
		        <?php if(isset($config['product'][$type]['photo2']) && $config['product'][$type]['photo2'] == true) { ?>
			        <div class="card card-primary card-outline text-sm">
			            <div class="card-header">
			                <h3 class="card-title">Hình ảnh <?=isset($config['product'][$type]['title_photo2']) ?$config['product'][$type]['title_photo2']: ++$count_other_photo?></h3>
			                <div class="card-tools">
			                	<button type="button" class="btn btn-tool" data-card-widget="collapse"><i class="fas fa-minus"></i></button>
			                </div>
			            </div>
				...
				<?php if(isset($config['product'][$type]['photo3']) && $config['product'][$type]['photo3'] == true) { ?>
			        <div class="card card-primary card-outline text-sm">
			            <div class="card-header">
			                <h3 class="card-title">Hình ảnh <?=isset($config['product'][$type]['title_photo3']) ?$config['product'][$type]['title_photo3']: ++$count_other_photo?></h3>
			                <div class="card-tools">
			                	<button type="button" class="btn btn-tool" data-card-widget="collapse"><i class="fas fa-minus"></i></button>
			                </div>
			            </div>
		        ...
		        <?php if(isset($config['product'][$type]['photo4']) && $config['product'][$type]['photo4'] == true) { ?>
			        <div class="card card-primary card-outline text-sm">
			            <div class="card-header">
			                <h3 class="card-title">Hình ảnh <?=isset($config['product'][$type]['title_photo4']) ?$config['product'][$type]['title_photo4']: ++$count_other_photo?></h3>
			                <div class="card-tools">
			                	<button type="button" class="btn btn-tool" data-card-widget="collapse"><i class="fas fa-minus"></i></button>
			                </div>
			            </div>
			            <div class="card-body">
	                    	<?php
	                    		$photoDetail = ($act != 'copy') ? UPLOAD_PRODUCT.@$item['photo4'] : '';
	                    		$dimension = "Width: ".$config['product'][$type]['width']." px - Height: ".$config['product'][$type]['height']." px (".$config['product'][$type]['img_type'].")";
	                    		$image_target="photo4";
	                    		$image_target_arr[]=$image_target;
	                    		include TEMPLATE.LAYOUT."image_other.php";
                ...
                <div class="row">
					...
					<?php if(isset($config['product'][$type]['gia']) && $config['product'][$type]['gia'] == true) { ?>
				    	<div class="form-group col-md-4">
	                        ...
	                        <div class="input-group">
	                        	<input type="text" class="form-control format-price gia_moi" name="data[gia]" id="gia" placeholder="Giá bán" value="<?=@$item['gia']?>">
				    ...
				    <?php if(isset($config['product'][$type]['giacu']) && $config['product'][$type]['giacu'] == true) { ?>
				    	<div class="form-group col-md-4">
	                        ...
	                        <div class="input-group">
	                        	<input type="text" class="form-control format-price gia_ban" name="data[giacu]" id="giacu" placeholder="Giá cũ" value="<?=@$item['giacu']?>">
	                        	<div class="input-group-append">
	                        		<div class="input-group-text"><strong>VNĐ</strong></div>
	                        	</div>
	                        </div>
				    ...
				    <?php if(isset($config['product'][$type]['iframe']) && $config['product'][$type]['iframe'] == true) { ?>
	                    <div class="form-group col-md-12">
	                        <label class="d-block" for="masp">Iframe:</label>
	                        <textarea class="form-control" name="data[iframe]" id="iframe" cols="30" rows="5" placeholder="Iframe code"><?=@$item['iframe']?></textarea>
	                    </div>
	                <?php } ?>
        ...
        <?php if(isset($config['product'][$type]['sub_gallery']) && $config['product'][$type]['sub_gallery'] == true) { ?>
        	<div class="card card-primary card-outline text-sm">
        		<div class="card-header">
