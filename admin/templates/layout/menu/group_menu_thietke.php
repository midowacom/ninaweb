...
if(isset($config['design'])) { 
    ...
    <?php
        ...
        if(isset($kiemtra) && $kiemtra == true) 
            if($func->check_access('design', 'man_price', $k, null, 'phrase-1') && $func->check_access('design', 'man_list', $k, null, 'phrase-1') && $func->check_access('design', 'man_cat', $k, null, 'phrase-1') && $func->check_access('design', 'man_item', $k, null, 'phrase-1') && $func->check_access('design', 'man_sub', $k, null, 'phrase-1') && $func->check_access('design', 'man_brand', $k, null, 'phrase-1') && $func->check_access('design', 'man', $k, null, 'phrase-1') && $func->check_access('import', 'man', $k, null, 'phrase-1') && $func->check_access('export', 'man', $k, null, 'phrase-1')) 
                $none = "d-none";
        if($com=='design')
        {
            $active = 'active';
        ...
        <li class="nav-item has-treeview <?=$menuopen?> <?=$none?>">
            ...
            <ul class="nav nav-treeview">
                ...
                <?php if($v=='designao'){ 
                    ...
                    <li class="nav-item <?=$none?>"><a class="nav-link <?=$active?>" href="index.php?com=newsletter&act=man&type=<?=$k?>" title="Đăng ký Áo Thiết kếiá"><i class="nav-icon text-sm far fa-caret-square-right"></i><p>Đăng ký Áo Thiết kế</p></a></li>
                <?php } ?>
                ...
                if($com=='newsletter' && ($act=='man' || $act=='add' || $act=='edit' || $act=='copy' || $kind=='man') && $k==$_GET['type']) $active = "active";
                ?>
