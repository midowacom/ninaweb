<?php
	...
	switch($act)
	{
		case "man":
			...
			if($type=='thietkeao') $template = "newsletter/man_thietkeao/items";
			break;

		case "add":
			$template = "newsletter/man/item_add";
			if($type=='thietkeao') $template = "newsletter/man_thietkeao/item_add";
			break;
	...
	function save_item()
	{
		...
		if($id)
		{
			...
			$data['ngaysua'] = time();
			if($type=="thietkeao"){
				$data['options'] = json_encode($_POST['soluong']);
				$data_soluong = array_values($_POST['soluong']);
