...
function save_item()
{
	global $d, $func, $curPage,$class_product;
	...
	if($id)
	{
		...
		if($d->update('order',$data)){
			...
			if($data['tinhtrang']==2){
				$chitietdonhang = $d->rawQuery("select id,id_product,id_mau,id_size,id_order,soluong from #_order_detail where id_order = ? order by id desc",array($id));
				foreach($chitietdonhang as $ctdonhang){
					$code = 'p'.$ctdonhang['id_product'].'-'.'c'.$ctdonhang['id_mau'].'-'.'s'.$ctdonhang['id_size'];
					$raw_stat = $d->rawQueryOne("select * from #_product_statistics where code=?",array($code));
					if($raw_stat['quantity'] >= $ctdonhang['soluong']){
						$data_quan['quantity'] = $raw_stat['quantity'] - $ctdonhang['soluong'];
					}
					$d->where('id', $raw_stat['id']);
					if($d->update('product_statistics',$data_quan)){
