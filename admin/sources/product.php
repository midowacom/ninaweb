<?php
	...
	switch($act)
	{
		...
		break;
		case "statistics":
			include "product_statistics.php";
		break;
		case "quanprice":
			include "product_price_byQuantity.php";
		break;
	...
	function save_item()
	{
		...
		if($data)
		{
			...
			if(isset($config['product'][$type]['brand']) && $config['product'][$type]['brand'] == true)
			{
		...
		if($id && $act!='save_copy')
		{
			...
			if(isset($_FILES['photo2']))
			{
				...
				if($photo2 = $func->uploadImage("photo2", $config['product'][$type]['img_type'], UPLOAD_PRODUCT,$file_name2))
				{
					$data['photo2'] = $photo2;
			...
			if(isset($_FILES['photo3']))
			{
				...
				if($photo3 = $func->uploadImage("photo3", $config['product'][$type]['img_type'], UPLOAD_PRODUCT,$file_name3))
				{
					$data['photo3'] = $photo3;
			...
			if(isset($_FILES['photo4']))
			{
				...
				if($photo4 = $func->uploadImage("photo4", $config['product'][$type]['img_type'], UPLOAD_PRODUCT,$file_name4))
				{
					$data['photo4'] = $photo4;
	...
	function delete_item()
	{
		...
		if($id)
		{
			...
			/* Lấy dữ liệu */
			$row = $d->rawQueryOne("select id, photo,photo2,photo3,photo4 from #_product where id = ? and type = ? limit 0,1",array($id,$type));
